...
class UserManager(models.Manager):
    ...
    def deleteFriend(self, user_id, friend_id):
        ...
        connection1 = Friend.objects.get(added_user=user, main_user=friend)
        connection2 = Friend.objects.get(added_user=friend, main_user=user)
        connection1.delete()
        connection2.delete()
    ...
    def addFriend(self, user_id, friend_id):
        user = self.get(id=user_id)
        friend = self.get(id=friend_id)
        Friend.objects.create(added_user=user, main_user=friend)
        Friend.objects.create(added_user=friend, main_user=user)
...
class Friend(models.Model):
    added_user = models.ForeignKey(User, related_name="added_friend")
    main_user = models.ForeignKey(User, related_name="current_user")
